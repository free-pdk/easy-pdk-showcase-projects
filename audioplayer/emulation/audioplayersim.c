...
#include "../audioplayer.c"                         // include the original source
...
void pdkdelay(uint32_t msec) {
  SDL_Delay(msec/5);                                //speedup delays in emulation
  INTRQ = INTEN; interrupt();                       //simulate (all enabled) interrupts
}

static uint32_t emu_spi_byte;

//emulation of IC version of pdkspi_init
void pdkspi_init(void) {
  emu_spi_byte = 0;
}

//emulation of IC version of pdkspi_sendreceive
uint8_t pdkspi_sendreceive(uint8_t s) {
  if( emu_spi_byte > sizeof(sound_raw) )
    return 0xFF;
...
int main(int argc, char* argv[]) {
  ...
  SDL_AudioSpec sdlas = { .freq=31250, .format=AUDIO_U8, .channels=1, .samples=WAV_BUFFER_SIZE, .callback=_emu_AudioCallback };
